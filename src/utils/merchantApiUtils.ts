...
export const regenerateApiKey = async (): Promise<string | null> => {
  try {
    ...
    // Log activity manually instead of using activity_logs table
    // since it doesn't seem to be in the schema
    console.info('API key regenerated by user:', user.email);
...
export const updateMerchantProfile = async (updates: Record<string, any>): Promise<boolean> => {
  try {
    ...
    // Update the merchant profile without directly specifying updated_at
    // since it doesn't appear to be in the schema
    const { error: updateError } = await supabase
      .from('merchant_profiles')
      .update(safeUpdates)
      .eq('id', user.id);
