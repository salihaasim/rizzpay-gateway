...
const LandingNavbar = () => {
  ...
  const handleGetStartedClick = () => {
    trackUserAction('get_started_button_click', {
      location: 'navbar'
    });
  };
  const handleTermsClick = () => {
    trackUserAction('terms_link_click', {
      location: 'navbar'
    });
  };
      ...
      <div className="flex h-14 max-w-screen-2xl items-center justify-between px-4 md:px-6 mx-auto">
        ...
        <div className="flex items-center gap-4">
          <nav className="hidden md:flex gap-6">
            <Link to="/terms" className="text-sm font-medium text-muted-foreground hover:text-[#0052FF] transition-colors flex items-center" onClick={handleTermsClick}>
