...
export class BankIntegrationService {
  ...
  static async initiatePayout(payoutRequest: PayoutRequest, bankCode: string = 'hdfc'): Promise<BankResponse> {
    try {
      ...
      const bankResponse = await this.simulateBankAPICall(payoutRequest, bankConfig);
      
      // Update payout with bank response
      await supabase
        .from('payout_requests')
        .update({
          bank_reference_id: bankResponse.bank_reference_id,
          status: bankResponse.status === 'accepted' ? 'processing' : 'failed',
          failure_reason: bankResponse.success ? null : bankResponse.message
        })
        .eq('id', payoutRequest.id);
